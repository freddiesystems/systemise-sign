...
test('[DOCUMENT_FLOW]: should be able to approve a document', async ({ page }) => {
  ...
  const { recipients } = await seedPendingDocumentWithFullFields({
    owner: user,
    teamId: team.id,
    recipients: ['user@systemise.dev', 'approver@systemise.dev'],
    recipientsCreateOptions: [
      {
        email: 'user@systemise.dev',
        role: RecipientRole.SIGNER,
      },
      {
        email: 'approver@systemise.dev',
...
test('[DOCUMENT_FLOW]: should be able to create, send with redirect url, sign a document and redirect to redirect url', async ({
  ...
  await page.getByLabel('Redirect URL').fill('https://systemise.dev');
  ...
  await page.waitForURL('https://systemise.dev');
