import { seedBlankDocument, seedDocuments, seedTeamDocuments } from '@systemise/prisma/seed/documents';
...
test('[TEAMS]: check team documents count with external team email', async ({ page }) => {
  ...
  const teamEmail = `external-team-email-${team.id}@test.systemise.dev`;
