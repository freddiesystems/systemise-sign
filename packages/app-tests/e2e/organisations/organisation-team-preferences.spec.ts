...
import { DocumentVisibility } from '@systemise/prisma/client';
import { seedTeamDocumentWithMeta } from '@systemise/prisma/seed/documents';
...
test('[ORGANISATIONS]: manage branding preferences', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Brand Website' }).fill('https://systemise.dev');
  ...
  expect(teamSettings.brandingUrl).toEqual('https://systemise.dev');
  ...
  expect(inheritedTeamSettings.brandingUrl).toEqual('https://systemise.dev');
...
test('[ORGANISATIONS]: manage email preferences', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Reply to email' }).fill('organisation@systemise.dev');
  ...
  // Check that the team settings have inherited these values.
  expect(teamSettings.emailReplyTo).toEqual('organisation@systemise.dev');
  ...
  // Check that the team settings now inherit from organisation again.
  expect(inheritedTeamSettings.emailReplyTo).toEqual('organisation@systemise.dev');
  ...
  expect(documentMeta.emailReplyTo).toEqual('organisation@systemise.dev');
