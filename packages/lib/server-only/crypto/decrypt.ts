import { SYSTEMISE_ENCRYPTION_SECONDARY_KEY } from '@systemise/lib/constants/crypto';
import { ZEncryptedDataSchema } from '@systemise/lib/server-only/crypto/encrypt';
...
export const decryptSecondaryData = (encryptedData: string): string | null => {
  if (!SYSTEMISE_ENCRYPTION_SECONDARY_KEY) {
    throw new Error('Missing encryption key');
  }

  try {
    const decryptedBufferValue = symmetricDecrypt({
      key: SYSTEMISE_ENCRYPTION_SECONDARY_KEY,
      data: encryptedData,
    });
