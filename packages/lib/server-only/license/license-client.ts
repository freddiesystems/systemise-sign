...
const LICENSE_KEY = env('NEXT_PRIVATE_SYSTEMISE_LICENSE_KEY');
const LICENSE_SERVER_URL = env('INTERNAL_OVERRIDE_LICENSE_SERVER_URL') || 'https://license.documenso.com';

declare global {
  // eslint-disable-next-line no-var
  var __systemise_license_client__: LicenseClient | undefined;
}
...
export class LicenseClient {
  ...
  public static async start(): Promise<void> {
    if (globalThis.__systemise_license_client__) {
      return;
    }

    const instance = new LicenseClient();

    globalThis.__systemise_license_client__ = instance;
  ...
  public static getInstance(): LicenseClient | null {
    return globalThis.__systemise_license_client__ ?? null;
  }
