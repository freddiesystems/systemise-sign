...
This document defines the writing conventions for Systemise documentation.
...
1. **Users** - People using the Systemise web application to send and sign documents
2. **Developers** - Building integrations with the API or SDKs
3. **Self-hosters** - Running their own Systemise instance
...
```typescript
// Basic usage
const document = await systemise.documents.create({
  title: "Contract",
  file: pdfBuffer,
});
