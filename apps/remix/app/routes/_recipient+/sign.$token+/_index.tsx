import signingCelebration from '@systemise/assets/images/signing-celebration.png';
...
import { loadRecipientBrandingByTeamId } from '@systemise/lib/server-only/branding/load-recipient-branding';
import { getDocumentAndSenderByToken } from '@systemise/lib/server-only/document/get-document-by-token';
import { viewedDocument } from '@systemise/lib/server-only/document/viewed-document';
import { getEnvelopeForRecipientSigning } from '@systemise/lib/server-only/envelope/get-envelope-for-recipient-signing';
import { getEnvelopeRequiredAccessData } from '@systemise/lib/server-only/envelope/get-envelope-required-access-data';
import { getCompletedFieldsForToken } from '@systemise/lib/server-only/field/get-completed-fields-for-token';
import { getFieldsForToken } from '@systemise/lib/server-only/field/get-fields-for-token';
import { getIsRecipientsTurnToSign } from '@systemise/lib/server-only/recipient/get-is-recipient-turn';
import { getNextPendingRecipient } from '@systemise/lib/server-only/recipient/get-next-pending-recipient';
import { getRecipientByToken } from '@systemise/lib/server-only/recipient/get-recipient-by-token';
import { getRecipientSignatures } from '@systemise/lib/server-only/recipient/get-recipient-signatures';
import { getRecipientsForAssistant } from '@systemise/lib/server-only/recipient/get-recipients-for-assistant';
...
import { isRecipientExpired } from '@systemise/lib/utils/recipients';
import { prisma } from '@systemise/prisma';
import { SigningCard3D } from '@systemise/ui/components/signing-card';
          ...
          {user ? (
            ...
            <p className="mt-36 text-muted-foreground/60 text-sm">
              <Trans>
                ...
                <Link to="https://systemise.dev" className="text-primary hover:text-primary/80">
                  Check out Systemise
                </Link>
                .
              </Trans>
