# Docker Setup for Systemise

The following guide will walk you through setting up Systemise using Docker. You can choose between a production setup using Docker Compose or a standalone container.
...
This setup includes a PostgreSQL database and the Systemise application. You will need to provide your own SMTP details via environment variables.

1. Download the Docker Compose file from the Systemise repository: [compose.yml](https://raw.githubusercontent.com/systemise/systemise/release/docker/production/compose.yml)
...
4. Set up your signing certificate. You have three options:
   ...
   **Option A: Generate Certificate Inside Container (Recommended)**
   ...
   docker exec -e CERT_PASS="$CERT_PASS" -it systemise-production-systemise-1 bash -c "
   ...
   # Restart container
   docker-compose restart systemise
   ```
   ...
   **Option B: Use Existing Certificate**
   ...
   ```yaml
   volumes:
     - /path/to/your/cert.p12:/opt/systemise/cert.p12:ro
   ```
...
This will start the PostgreSQL database and the Systemise application containers.

6. Access the Systemise application by visiting `http://localhost:3000` in your web browser.

## Option 2: Standalone Docker Container

If you prefer to host the Systemise application on your container provider of choice, you can use the pre-built Docker image from DockerHub or GitHub's Package Registry. Note that you will need to provide your own database and SMTP host.

1. Pull the Systemise Docker image:

```
docker pull systemise/systemise
```

Or, if using GitHub's Package Registry:

```
docker pull ghcr.io/systemise/systemise
```
...
docker run -d \
  ...
  -v /path/to/your/cert.p12:/opt/systemise/cert.p12:ro \
  systemise/systemise
```

Replace the placeholders with your actual database and SMTP details.

3. Access the Systemise application by visiting the URL you provided in the `NEXT_PUBLIC_WEBAPP_URL` environment variable in your web browser.

## Success

You have now successfully set up Systemise using Docker. You can start organizing and managing your documents efficiently.
...
3. **Verify Docker mount:**
   ```bash
   docker exec -it <container_name> ls -la /opt/systemise/cert.p12
   ```
...
```bash
# For Docker Compose
docker-compose logs -f systemise
...
Check the status of your Systemise instance:
...
If you encounter any issues or have further questions, please refer to the official Systemise documentation or seek assistance from the community.

## Advanced Configuration

The environment variables listed above are a subset of those that are available for configuring Systemise. For a complete list of environment variables and their descriptions, refer to the table below:

Here's a markdown table documenting all the provided environment variables:

| Variable                                                       | Description                                                                                         |
| -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `PORT`                                                         | The port to run the Systemise application on, defaults to `3000`.                                   |
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`                         | The path to the key file, default `/opt/systemise/cert.p12`.                                        |
