...
if [ ! -z "$DOCKER_REPOSITORY" ]; then
    ...
    docker tag "systemise-base" "$DOCKER_REPOSITORY:latest"
    docker tag "systemise-base" "$DOCKER_REPOSITORY:$GIT_SHA"

    # Add version tag if available
    if [ ! -z "$APP_VERSION" ] && [ "$APP_VERSION" != "undefined" ]; then
        docker tag "systemise-base" "$DOCKER_REPOSITORY:$APP_VERSION"
    fi
else
    echo "Using default repositories: dockerhub and ghcr.io"
    
    # Add tags for both default repositories
    docker tag "systemise-base" "systemise/systemise:latest"
    docker tag "systemise-base" "systemise/systemise:$GIT_SHA"
    docker tag "systemise-base" "ghcr.io/systemise/systemise:latest"
    docker tag "systemise-base" "ghcr.io/systemise/systemise:$GIT_SHA"

    # Add version tags if available
    if [ ! -z "$APP_VERSION" ] && [ "$APP_VERSION" != "undefined" ]; then
        docker tag "systemise-base" "systemise/systemise:$APP_VERSION"
        docker tag "systemise-base" "ghcr.io/systemise/systemise:$APP_VERSION"
    fi
fi

# Remove the temporary base tag
docker rmi "systemise-base"
